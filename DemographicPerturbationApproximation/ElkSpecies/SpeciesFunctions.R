...
f_function <- function() {
  ...
  # For the baseline SI_A should be 1 for all seasons all nodes SI_A remains as a perturbation perameter.
  ...
  if(ind == 2){ # summer
    if(type == 1){ # calves
      repro <- 0.6*R*SI_A*Na # Number of female calves born
      }
    if(type == 2){ # adults
      calves_survival <- SI_C*exp(1-(Na+Nc)/(K)) # Density dependent survival
      trans <-calves_survival*Nc # Number of calves transitioning to adult
    }
  }
